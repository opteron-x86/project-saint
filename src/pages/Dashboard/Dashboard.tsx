...
import { LoadingIndicator, ErrorDisplay, Card } from '@/components/common'; // Assuming Card is a common component
import { SEVERITY_DISPLAY, SEVERITY_COLORS, PLATFORM_COLORS, RULE_SOURCE_COLORS } from '@/utils/constants'; // Import constants for display
...
const RecentActivityPanel: React.FC = () => {
    const { rules, isLoading, isError, error } = usePaginatedRules(1, 5);
    const navigate = useNavigate();
    const handleRuleClick = (rule: RuleSummary) => navigate('/rules', { state: { initialSearchTerm: `id:"${rule.id}"` } });
=======
